...
class MultiGraphData(Data):
    def __init__(self, x=None, edge_index=None, edge_attr=None,
                 fg_x=None, fg_edge_index=None, atom2fg_index=None, subgraphs = None,
                 **kwargs):
...
class PretrainDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        for smiles_id, smiles, fgs in tqdm(zip(smiles_id_list,smiles_list, mol2fgs)):
            ...
            atom_features, bond_list, bond_features, fg_features, fg_edge_list, fg_edge_features, atom2fg_list, subgraphs = mol_to_graphs(mol)
...
class MoleculeNetDataset():
    ...
    def __getitem__(self, index):
        atom_features, bond_list, bond_features, fg_features, fg_edge_list, fg_edge_features, atom2fg_list, subgraphs, y, w = self.dataset[index]
        data = MultiGraphData(x=torch.Tensor(atom_features),
                              edge_index=torch.LongTensor(bond_list).reshape(-1, 2).transpose(1, 0),
                              edge_attr=torch.Tensor(bond_features).reshape(-1, BOND_DIM),
                              fg_x=torch.Tensor(fg_features),

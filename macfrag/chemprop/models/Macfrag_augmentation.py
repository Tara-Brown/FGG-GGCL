...
class FragmentGNN(nn.Module):
    def __init__(self, args: Namespace, input_dim=73, hidden_per_head=32, num_layers=3, heads=4):
    ...
    def forward(self, data):
        ...
        #print("🔍 FragmentGNN input x.shape:", x.shape)
...
class SerGINE_MacFrag(nn.Module):
    def __init__(self, num_atom_layers=3, num_fg_layers=2, latent_dim=128,
                 atom_dim=73, fg_dim=73, bond_dim=11, fg_edge_dim=73, fragment_gnn=None,
                 atom2fg_reduce='mean', pool='mean', dropout=0, device='cuda:1', **kwargs):
        super().__init__()
    ...
    def forward(self, data):
        ...
        #print(fg_x.shape)
        # Move atom data to device
        atom_x = atom_x.to(self.device)
        atom_edge_index = atom_edge_index.to(self.device)
        atom_edge_attr = atom_edge_attr.to(self.device)
        atom_batch = atom_batch.to(self.device)

        # Move fg data to device
        fg_x = fg_x.to(self.device)
        fg_edge_index = fg_edge_index.to(self.device)
        fg_edge_attr = fg_edge_attr.to(self.device)
        fg_batch = fg_batch.to(self.device)

        # Batch subgraphs and move to device
        flat_subgraphs = [fg for mol_subgraphs in subgraphs for fg in mol_subgraphs]
        batched_fgs = Batch.from_data_list(flat_subgraphs).to(self.device)

        #print("🧩 Number of fragment subgraphs:", len(flat_subgraphs))
        #print("🧩 First fragment node feature shape:", flat_subgraphs[0].x.shape)

        fg_embeds = self.fragment_gnn(batched_fgs)

        # Embed fragment features and add fragment GNN embeddings
...
class FragmentGNNEncoder_MacFrag(nn.Module):
    def __init__(self, args: Namespace, num_tasks=1):
        ...
        self.encoder = SerGINE_MacFrag(
            ...
            device = args.device
        ).to(args.device)  # Ensure encoder is also moved to device

    def forward(self, data):
        df = pd.DataFrame(data)
        new_data = process_data(df, self.args)
...
def process_data(df, args):
    ...
    for i in range(len(df)):
        ...
        #print(f"Sample {i} fg_features shape: ({len(fg_features)}, {len(fg_features[0]) if fg_features else 'N/A'})")
        if len(fg_features) == 0:
            ...
            dummy_fg = Data(
                x=torch.zeros((1, args.fg_dim)),  # match fg_dim
                edge_index=torch.empty((2, 0), dtype=torch.long)
            )
            fragment_subgraphs = [dummy_fg]

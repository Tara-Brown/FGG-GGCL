...
class MultiGraphData(Data):
    def __init__(self, x=None, edge_index=None, edge_attr=None,
                 fg_x=None, fg_edge_index=None, atom2fg_index=None, fragment_subgraphs=None,
                 **kwargs):
        ...
        self.subgraphs = fragment_subgraphs #new
    ...
    def __inc__(self, key, value, store):
        ...
        if key == 'fg_edge_index':
            ...
            return super(MultiGraphData, self).__inc__(key, value, store)
...
class PretrainDataset(InMemoryDataset):
    ...
    def process(self):
        with open(self.raw_paths[0], 'r') as f:
            ...
            smiles_id_list = list(range(len(smiles_list)))
        ...
        data_list = []
        for smiles_id, smiles, fgs in tqdm(zip(smiles_id_list,smiles_list, mol2fgs)):
            ...
            data = MultiGraphData(x=torch.Tensor(atom_features),
                                  ...
                                  fragment_subgraphs=fragment_subgraphs, #new
                                  fp=torch.Tensor(fpvec).reshape(1, -1),
                                  fg=torch.Tensor(fgvec).reshape(1, -1),smiles_id=smiles_id)
...
class MoleculeNetDataset():
    ...
    def __getitem__(self, index):
        atom_features, bond_list, bond_features, fg_features, fg_edge_list, fg_edge_features, atom2fg_list, fragment_subgraphs, y, w = self.dataset[index]
        data = MultiGraphData(x=torch.Tensor(fg_features), #changed temp
                              fg_x = torch.Tensor(fg_features), #changed temp
                              edge_index=torch.LongTensor(bond_list).reshape(-1, 2).transpose(1, 0),
                              edge_attr=torch.Tensor(bond_features).reshape(-1, BOND_DIM),
                              fg_edge_index=torch.LongTensor(fg_edge_list).reshape(-1, 2).transpose(1, 0),
                              fg_edge_attr=torch.Tensor(fg_edge_features).reshape(-1, FG_EDGE_DIM),
                              #atom2fg_index=torch.LongTensor(atom2fg_list).reshape(-1, 2).transpose(1, 0),
                              fragment_subgraphs=fragment_subgraphs, #new
                              y=torch.from_numpy(np.array(y).astype(float)).to(torch.float32),
                              w=torch.Tensor(w))
        return data
...
class DDIDataset(InMemoryDataset):
    ...
    def process(self):
        df = pd.read_csv(self.raw_paths[0], usecols=['smiles'])

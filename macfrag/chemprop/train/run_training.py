...
# =============================================================================
    ...
    val_mol, test_mol, train_mol = split_data(
        data=mol,
    ...
    train_smiles, train_targets = train_mol.smiles(), train_mol.targets()
    val_smiles, val_targets = val_mol.smiles(), val_mol.targets()
    test_smiles, test_targets = test_mol.smiles(), test_mol.targets()
    train_dataset = SmilesTargetDataset(train_smiles, train_targets)
    val_dataset   = SmilesTargetDataset(val_smiles, val_targets)
    test_dataset  = SmilesTargetDataset(test_smiles, test_targets)
    ...
    for model_idx in range(args.ensemble_size):
        ...
        for epoch in range(args.start_epochs,args.end_epochs):
            ...
            with tqdm(total=len(train_loader)) as t:
                for step, (batch, target) in enumerate(train_loader):
                    batch = tuple(
                        b.to(device) if hasattr(b, 'to') else b
                        for b in batch
                    )
                    '''print(f"batch device check:")
                    for i, b in enumerate(batch):
                        if hasattr(b, 'device'):
                            print(f" batch[{i}] device: {b.device}")
                        else:
                            print(f" batch[{i}] has no device attribute")
                    ...
                    #model1.train()
                    model4.train()
                    #print(f"model3 device: {next(model3.parameters()).device}")
                    #print(f"model4 device: {next(model4.parameters()).device}")
                    if args.cuda:
                        #print('Moving model3 and model4 to CUDA device')
                        model3 = model3.to(device)  # device is cuda:1 here
                        #model1 = model1.to(device)  # MUST move model4 also!
                        model4 = model4.to(device)
                    emb1 = model3(batch) # Group Graph
                    #emb2 = model2(batch)
                    emb2 = model4(step, False, batch, None) # Functional Group graph + y (for now)

                    #print(f"emb1 device: {emb1.device}")
                    #print(f"emb2 device: {emb2.device}")
            ...
            for step, (test_batch, test_target) in enumerate(val_loader):
                with torch.no_grad():
                    test_batch = tuple(
                        b.to(device) if hasattr(b, 'to') else b
                        for b in test_batch
                    )
                    #print(f"test_batch device check:")
                    '''
                    for i, b in enumerate(test_batch):
                        if hasattr(b, 'device'):
                            print(f" test_batch[{i}] device: {b.device}")
                        else:
                            print(f" test_batch[{i}] has no device attribute")
                    '''
                    #print(f"test_target device: {test_target.device}")
                    if args.cuda:
                        #print('Moving model3 and model4 to CUDA device')
                        model3 = model3.to(device)  # device is cuda:1 here
                        #model1 = model1.to(device)  # MUST move model4 also!
                    ...
                    #print(f"test emb1 device: {emb1.device}")
                    #print(f"test emb2 device: {emb2.device}")

                    labels = test_target.to(device).view(-1, args.num_tasks)
                    #print(f"test labels device: {labels.device}")
                    labels = test_target.to(device).view(-1, args.num_tasks)
                    mask = ~torch.isnan(labels)
            ...
            #snapshot(model1, epoch, dump_folder, 'model1')
            snapshot(model4, epoch, dump_folder, 'CMGNN')
            ...
            if test_auc > best_auc:
                ...
                #torch.save(model1.state_dict(), f'{dump_folder}/best_model_model1.pt')
                torch.save(model4.state_dict(), f'{dump_folder}/best_model_CMGNN.pt')
            else:
                epochs_no_improve += 1
        ...
        for test_batch, test_target in test_loader:
            with torch.no_grad():
                ...
                y_score_test.append(torch.sigmoid(output).cpu().numpy())
        avg_test_loss = total_test_loss / len(test_loader)
        y_true_np = np.concatenate(y_true_test, axis=0)
        y_score_np = np.concatenate(y_score_test, axis=0)
        test_auc = masked_roc_auc_score(y_true_np, y_score_np)
        logger.info(f"[FINAL TEST] Best Epoch = {best_epoch} | "
                    f"Test ROC-AUC = {test_auc:.4f} | "
                    f"Avg Test Loss = {avg_test_loss:.4f}")
